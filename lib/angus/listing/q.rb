...
module Angus
  module Listing
    ...
    class Q
      ...
      # @param [Hash<Symbol>] columns Columns allowed to be filter and sorted by
    ...
    #  - or
    #
    # Please, look at {Angus::Listing::Conditions} for further reference.
    class QFilter
      ...
      def get_affected_tables(options, columns)
        options.map do |option|
          ...
          elsif columns.has_key?(option.to_sym) && option =~ /(.*)\./
            $1.to_sym
          end
        end.flatten.compact
      end
    ...
    # Sorting behavior for a Angus::Listing::Dataset
    #
    ...
    class QSorting
      ...
      def escape_names(options, columns, ns)
        options.map do |option|
          if columns.has_key?(option.to_sym) && columns[option.to_sym]
            columns[option.to_sym]
          elsif columns.has_key?(option.to_sym)
            option = Utils.add_namespace(option, ns)
            "`#{option.to_s.gsub('.','`.`')}`"
